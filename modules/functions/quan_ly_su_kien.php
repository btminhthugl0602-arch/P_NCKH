...
function btc_tao_su_kien($conn, $id_nguoi_tao, $ten_su_kien, $mo_ta, $id_cap, $ngay_mo_dk = null, $ngay_dong_dk = null, $ngay_bat_dau = null, $ngay_ket_thuc = null, $is_active = 1)
{
    ...
    // Sao chép các vai trò toàn cục → vaitro_sukien của sự kiện này
    // =========================================================
    ...
    // Lấy vai trò BTC (idVaiTroGoc = 1) vừa tạo cho sự kiện này
    $sql_get_btc = "SELECT idVaiTroSK FROM vaitro_sukien
                    WHERE idSK = $idSK AND idVaiTroGoc = 1 LIMIT 1";
    $res_btc = mysqli_query($conn, $sql_get_btc);

    if ($res_btc && mysqli_num_rows($res_btc) > 0) {
        $idVaiTroSK_BTC = (int)mysqli_fetch_assoc($res_btc)['idVaiTroSK'];

        $sql_assign = "INSERT INTO taikhoan_vaitro_sukien
                           (idTK, idSK, idVaiTroSK, idVaiTroGoc, nguonTao, idNguoiCap, ngayCap, isActive)
                       VALUES
                           ($id_nguoi_tao, $idSK, $idVaiTroSK_BTC, 1, 'BTC_THEM', $id_nguoi_tao, NOW(), 1)";
        mysqli_query($conn, $sql_assign);
    }

    // Ghi vào bantochuc để các module khác dễ JOIN
    $sql_btc = "INSERT INTO bantochuc (idSK, idTK, chucVu) VALUES ($idSK, $id_nguoi_tao, 'BTC')";
    mysqli_query($conn, $sql_btc);
    // =========================================================
