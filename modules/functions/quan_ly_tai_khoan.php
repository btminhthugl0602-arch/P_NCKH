...
// ĐĂNG NHẬP
// ============================================================
function dang_nhap($conn, $ten_tk, $mat_khau): array
{
    // Validate đầu vào
    $ten_tk   = trim($ten_tk);
    ...
    if ($ten_tk === "" || $mat_khau === "") {
        return ['status' => false, 'message' => 'Vui lòng nhập đầy đủ thông tin!!'];
    }
    ...
    if (password_verify($mat_khau, $row['matKhau'])) {
        // Mật khẩu đã hash → OK
        $mat_khau_hop_le = true;
    } elseif ($mat_khau === $row['matKhau']) {
        // Mật khẩu cũ còn plaintext → tự động hash lại
        $new_hash = password_hash($mat_khau, PASSWORD_DEFAULT);
        _update_info(
            $conn,
            'taikhoan',
            ['matKhau'],
