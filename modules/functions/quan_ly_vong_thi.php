...
{
    // Check quyền sự kiện (cauhinh_vongthi) HOẶC quyền hệ thống tao_su_kien
    $has_perm = kiem_tra_quyen_su_kien($conn, (int)$id_nguoi_tao, (int)$id_sk, 'cauhinh_vongthi')
...
{
    $vt = truy_van_mot_ban_ghi($conn, 'VONGTHI', 'idVongThi', (int)$id_vong_thi);
    $id_sk_check = $vt ? (int)$vt['idSK'] : 0;
    $has_perm = ($id_sk_check > 0 && kiem_tra_quyen_su_kien($conn, (int)$id_nguoi_sua, $id_sk_check, 'cauhinh_vongthi'))
        || kiem_tra_quyen_he_thong($conn, $id_nguoi_sua, 'tao_su_kien');
    if (!$has_perm) {
...
function xoa_vong_thi($conn, $id_nguoi_xoa, $id_vong_thi)
{
    $vt = truy_van_mot_ban_ghi($conn, 'VONGTHI', 'idVongThi', (int)$id_vong_thi);
    $id_sk_check = $vt ? (int)$vt['idSK'] : 0;
    $has_perm = ($id_sk_check > 0 && kiem_tra_quyen_su_kien($conn, (int)$id_nguoi_xoa, $id_sk_check, 'cauhinh_vongthi'))
