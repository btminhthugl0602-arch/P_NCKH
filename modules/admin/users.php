        ...
        if ($idTK <= 0) {
            ...
            if ($idLoaiTK == 2) {
                ...
                if ($chk) {
                    $sv_f = ['tenSV'];
                    $sv_v = [$hoTen];
                    if (!empty($msv)) { $sv_f[] = 'MSV'; $sv_v[] = $msv; }
                    if (is_numeric($gpa)) { $sv_f[] = 'GPA'; $sv_v[] = (float)$gpa; }
                    if (is_numeric($drl)) { $sv_f[] = 'DRL'; $sv_v[] = (int)$drl; }
                    if ($idDonVi > 0) {
                        $sv_f[] = 'idLop';  $sv_v[] = $idDonVi;
                        $sv_f[] = 'idKhoa'; $sv_v[] = $idKhoaSV;
                    }
...
function group_quyen(array $quyens): array {
    $out = [];
    foreach ($quyens as $q) { $out[$q['tenNhom'] ?? 'Khác'][] = $q; }
    return $out;
}
...
function render_quyen_accordion(array $grouped, string $prefix, string $colorClass, bool $allOpen = false): void {
    ...
    foreach ($grouped as $nhom => $qs) {
        $i++; $colId = $prefix . '_g' . $i; $show  = $allOpen ? 'show' : '';
?>
        <div class="accordion-item border rounded-3 mb-2">
            <h2 class="accordion-header">
                <button class="accordion-button <?= $show ? '' : 'collapsed' ?> py-2 rounded-3" type="button" data-bs-toggle="collapse" data-bs-target="#<?= $colId ?>">
                    <i class="bi bi-folder me-2 text-<?= $colorClass ?>"></i><?= htmlspecialchars($nhom) ?>
                    <span class="badge bg-<?= $colorClass ?>-subtle text-<?= $colorClass ?> border border-<?= $colorClass ?> ms-2"><?= count($qs) ?></span>
                </button>
            </h2>
                ...
                <div class="accordion-body pt-1">
                    <?php foreach ($qs as $q): ?>
                        <div class="form-check mb-2">
                            <input class="form-check-input <?= $prefix ?>-cb" type="checkbox" name="<?= strpos($prefix, 'role') === false ? 'quyen_ids' : 'role_quyen_ids' ?>[]" value="<?= $q['idQuyen'] ?>" id="<?= $prefix ?>_<?= $q['idQuyen'] ?>" data-idquyen="<?= $q['idQuyen'] ?>">
                            <label class="form-check-label" for="<?= $prefix ?>_<?= $q['idQuyen'] ?>">
                                ...
                                <?php if (!empty($q['moTa'])): ?><div class="text-muted small"><?= htmlspecialchars($q['moTa']) ?></div><?php endif; ?>
                            </label>
                        </div>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="modal-body p-0">
                <div class="enrollment-form-wrapper">
                    <div class="enrollment-header text-center mb-4 pt-4 px-4 position-relative">
                        ...
                        <span class="badge bg-primary fs-6 px-3 py-2 d-inline-block mb-2"><i class="bi bi-person-plus me-1"></i>Tạo tài khoản mới</span>
                    ...
                    <form class="enrollment-form px-4 pb-4" method="POST" action="">
                        ...
                        <h6 class="fw-bold text-primary mb-3 border-bottom pb-2"><i class="bi bi-key me-1"></i>Thông tin đăng nhập</h6>
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Tên đăng nhập <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" name="tenTK" placeholder="VD: sv_nguyen..." required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Mật khẩu <span class="text-danger">*</span></label>
                                <div class="input-group">
                                    <input type="password" class="form-control" name="matKhau" id="createPassword" placeholder="Nhập mật khẩu..." required>
                                    <button type="button" class="btn btn-outline-secondary" onclick="togglePwd('createPassword',this)"><i class="bi bi-eye"></i></button>
                                </div>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Loại tài khoản <span class="text-danger">*</span></label>
                                <select class="form-select" name="idLoaiTK" id="createLoaiTK" onchange="toggleCreateForm(this.value)" required>
                                    <option value="">-- Chọn loại --</option>
                                    <?php foreach ($loai_tks as $ltk): ?>
                                        <option value="<?= $ltk['idLoaiTK'] ?>"><?= htmlspecialchars($ltk['tenLoaiTK']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <div class="d-flex gap-3 mt-2">
                                    <div class="form-check"><input class="form-check-input" type="radio" name="isActive" value="1" checked><label class="form-check-label"><i class="bi bi-unlock text-success me-1"></i>Hoạt động</label></div>
                                    <div class="form-check"><input class="form-check-input" type="radio" name="isActive" value="0"><label class="form-check-label"><i class="bi bi-lock text-danger me-1"></i>Khóa</label></div>
                                </div>
                            </div>
                        </div>

                        <div id="create-gv-section" class="d-none">
                            <h6 class="fw-bold text-success mb-3 border-bottom pb-2"><i class="bi bi-person-workspace me-1"></i>Hồ sơ giảng viên</h6>
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold">Họ và tên <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control gv-hoten" name="hoTen_gv" placeholder="Nhập họ và tên...">
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold">Giới tính</label>
                                    <div class="d-flex gap-2 mt-2">
                                        <div class="form-check"><input class="form-check-input" type="radio" name="gioiTinh" value="1"><label class="form-check-label">Nam</label></div>
                                        <div class="form-check"><input class="form-check-input" type="radio" name="gioiTinh" value="0" checked><label class="form-check-label">Nữ</label></div>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold">Khoa</label>
                                        <select class="form-select" name="idDonVi_gv" id="create-idKhoa">                                        <option value="">-- Chọn --</option>
                        ...
                        <div id="create-sv-section" class="d-none">
                            <h6 class="fw-bold text-info mb-3 border-bottom pb-2"><i class="bi bi-mortarboard me-1"></i>Hồ sơ sinh viên</h6>
                            <div class="row mb-3">
                                <div class="col-md-5">
                                    <label class="form-label fw-semibold">Họ và tên <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control sv-hoten" name="hoTen_sv" placeholder="Nhập họ và tên...">
                                </div>
                                <div class="col-md-3">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="number" step="0.01" min="0" max="4" class="form-control" name="gpa" placeholder="0.00">
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label fw-semibold">ĐRL</label>
                                    <input type="number" min="0" max="100" class="form-control" name="drl" placeholder="0">
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-semibold">Lớp</label>
                                    <select class="form-select" name="edit_idDonVi_sv" id="edit_idLop">                                    <option value="">-- Chọn lớp --</option>
                                    <?php foreach ($lops as $l): ?>
                                        <option value="<?= $l['idLop'] ?>"><?= htmlspecialchars($l['tenLop']) ?> (<?= htmlspecialchars($l['tenKhoa']) ?>)</option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <?php if (!empty($ht_grouped)): ?>
                            <h6 class="fw-bold text-warning mb-3 border-bottom pb-2"><i class="bi bi-shield-lock me-1"></i>Quyền hệ thống <small class="text-muted fw-normal">(tuỳ chọn)</small></h6>
                        ...
                        <div class="d-flex justify-content-between pt-3 mt-2 border-top">
                            ...
                            <button type="submit" class="btn btn-primary px-4"><i class="bi bi-check-circle me-1"></i>Tạo tài khoản</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal-content border-0">
            <div class="modal-body p-0">
                <div class="enrollment-form-wrapper">
                    <div class="enrollment-header text-center mb-4 pt-4 px-4 position-relative">
                        <button type="button" class="btn-close position-absolute top-0 end-0 m-3" data-bs-dismiss="modal"></button>
                        <span class="badge bg-warning text-dark fs-6 px-3 py-2 d-inline-block mb-2"><i class="bi bi-pencil-square me-1"></i>Chỉnh sửa tài khoản</span>
                    ...
                    <form class="enrollment-form px-4 pb-4" method="POST" action="">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Mật khẩu mới <small class="text-muted">(bỏ trống = không đổi)</small></label>
                                <div class="input-group">
                                    <input type="password" class="form-control" name="edit_matKhau" id="editPassword" placeholder="Mật khẩu mới...">
                                    <button type="button" class="btn btn-outline-secondary" onclick="togglePwd('editPassword',this)"><i class="bi bi-eye"></i></button>
                                </div>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="edit_idLoaiTK" id="edit_idLoaiTK" onchange="toggleEditForm(this.value)">
                                    <?php foreach ($loai_tks as $ltk): ?>
                                        <option value="<?= $ltk['idLoaiTK'] ?>"><?= htmlspecialchars($ltk['tenLoaiTK']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Trạng thái</label>
                                <div class="d-flex gap-3 mt-2">
                                    <div class="form-check"><input class="form-check-input" type="radio" name="edit_isActive" id="edit_active1" value="1"><label class="form-check-label" for="edit_active1"><i class="bi bi-unlock text-success me-1"></i>Hoạt động</label></div>
                                    <div class="form-check"><input class="form-check-input" type="radio" name="edit_isActive" id="edit_active0" value="0"><label class="form-check-label" for="edit_active0"><i class="bi bi-lock text-danger me-1"></i>Khóa</label></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="edit-gv-section" class="d-none">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold">Giới tính</label>
                                    <div class="d-flex gap-2 mt-2">
                                        <div class="form-check"><input class="form-check-input" type="radio" name="edit_gioiTinh" id="edit_gt1" value="1"><label class="form-check-label" for="edit_gt1">Nam</label></div>
                                        <div class="form-check"><input class="form-check-input" type="radio" name="edit_gioiTinh" id="edit_gt0" value="0"><label class="form-check-label" for="edit_gt0">Nữ</label></div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label fw-semibold">Khoa</label>
                        ...
                        <div id="edit-sv-section" class="d-none">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-2">
                                    <label class="form-label fw-semibold">GPA</label>
                                    <input type="number" step="0.01" min="0" max="4" class="form-control" name="edit_gpa" id="edit_gpa">
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label fw-semibold">ĐRL</label>
                                    <input type="number" min="0" max="100" class="form-control" name="edit_drl" id="edit_drl">
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-semibold">Lớp</label>
                        ...
                        <div class="d-flex justify-content-between pt-3 mt-2 border-top">
                            <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Hủy</button>
                            <button type="submit" class="btn btn-warning px-4"><i class="bi bi-save me-1"></i>Lưu thay đổi</button>
                        </div>
                    </form>
                </div>
            </div>
...
<div class="modal fade" id="permModal" tabindex="-1" aria-hidden="true">...</div>
<div class="modal fade" id="createGlobalRoleModal" tabindex="-1" aria-hidden="true">...</div>
<div class="modal fade" id="editGlobalRoleInfoModal" tabindex="-1" aria-hidden="true">...</div>
<div class="modal fade" id="editGlobalRolePermModal" tabindex="-1" aria-hidden="true">...</div>
    ...
    <section class="section">
        <div class="container" data-aos="fade-up" data-aos-delay="100">
            ...
            <ul class="nav nav-tabs mb-4 border-bottom">
                <li class="nav-item">
                    <a class="nav-link <?= $active_tab == 1 ? 'active' : '' ?>" href="?module=admin&action=users&tab=1<?= !empty($search) ? '&search=' . urlencode($search) : '' ?><?= $filter_role > 0 ? '&filter_role=' . $filter_role : '' ?>">
                        <i class="bi bi-people me-1"></i>Tài khoản <span class="badge bg-primary ms-1"><?= $total_u ?></span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link <?= $active_tab == 2 ? 'active' : '' ?>" href="?module=admin&action=users&tab=2">
                        <i class="bi bi-person-badge me-1"></i>Vai trò mẫu <span class="badge bg-success ms-1"><?= count($global_roles) ?></span>
                    </a>
                </li>
            </ul>
                            ...
                            <?php foreach ($users as $idx => $u): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php if (!empty($u['msv'])): ?><div class="text-muted small"><?= htmlspecialchars($u['msv']) ?></div><?php endif; ?>
                                    </td>
                                    <td><?= htmlspecialchars($don_vi ?: '—') ?></td>
                                    <td><span class="badge bg-<?= $rc ?>-subtle text-<?= $rc ?> border border-<?= $rc ?>"><i class="bi <?= $ri ?> me-1"></i><?= htmlspecialchars($u['tenLoaiTK'] ?? '—') ?></span></td>
                                    <td>
                                        <?php if ($u['isActive']): ?><span class="badge bg-success-subtle text-success border border-success"><i class="bi bi-check-circle me-1"></i>Hoạt động</span>
                                        <?php else: ?><span class="badge bg-danger-subtle text-danger border border-danger"><i class="bi bi-x-circle me-1"></i>Đã khóa</span><?php endif; ?>
                                    </td>
                                    <td class="text-muted small"><?= date('d/m/Y', strtotime($u['ngayTao'])) ?></td>
                                    <td class="text-center pe-3">
                                        <div class="d-flex justify-content-center gap-1">
                                            <button class="btn btn-sm btn-outline-warning" onclick='openEditModal(<?= $u_json ?>)' title="Chỉnh sửa"><i class="bi bi-pencil"></i> Sửa</button>
                                            <button class="btn btn-sm btn-outline-info" onclick="openPermModal(<?= $u['idTK'] ?>,'<?= addslashes($u['tenTK']) ?>')" title="Phân quyền hệ thống"><i class="bi bi-shield-lock"></i> Quyền</button>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
...
<script>
    ...
    // Các hàm JS khác giữ nguyên (openPermModal, openEditGlobalRoleInfoModal...)
</script>
