...
if ($is_btc && isPost()) {
    ...
    if ($act === 'tao_lich') {
        ...
        if ($tenHD && $thoiGian && $idSKPost) {
            ...
            if (mysqli_query($conn, $sql)) {
                $_SESSION['flash_msg'] = 'Đã thêm lịch trình thành công!'; $_SESSION['flash_type'] = 'success'; header('Location: ' . $_SERVER['REQUEST_URI']); exit;
            } else {
                $_SESSION['flash_msg'] = 'Lỗi khi thêm lịch trình: ' . mysqli_error($conn); $_SESSION['flash_type'] = 'danger'; header('Location: ' . $_SERVER['REQUEST_URI']); exit;
            }
        } else {
            $_SESSION['flash_msg'] = 'Vui lòng điền đầy đủ thông tin bắt buộc.'; $_SESSION['flash_type'] = 'warning'; header('Location: ' . $_SERVER['REQUEST_URI']); exit;
        }
    }
    ...
    if ($act === 'toggle_diemdanh') {
        ...
        if ($idLichPost) {
            if ($trangThai === 'mo') {
                ...
                $_SESSION['flash_msg'] = 'Đã đóng điểm danh!'; $_SESSION['flash_type'] = 'success'; header('Location: ' . $_SERVER['REQUEST_URI']); exit;
            }
    ...
    if ($act === 'diemdanh_tay') {
        ...
        if ($idLichPost && $idTKPost) {
            ...
            if (!$check) {
                ...
                $_SESSION['flash_msg'] = 'Đã điểm danh thủ công thành công!'; $_SESSION['flash_type'] = 'success'; header('Location: ' . $_SERVER['REQUEST_URI']); exit;
            } else {
                $_SESSION['flash_msg'] = 'Tài khoản này đã được điểm danh rồi.'; $_SESSION['flash_type'] = 'warning'; header('Location: ' . $_SERVER['REQUEST_URI']); exit;
            }
        }
    }
    ...
    if ($act === 'xoa_diemdanh') {
        ...
        if ($idDD) {
            ...
            $_SESSION['flash_msg'] = 'Đã xóa bản ghi điểm danh.'; $_SESSION['flash_type'] = 'success'; header('Location: ' . $_SERVER['REQUEST_URI']); exit;
        }
    }
}
            ...
            <div class="col-lg-8" data-aos="fade-up" data-aos-delay="200">
                ...
                <div class="course-details-card mb-4">
                    ...
                    <div class="row g-3 mb-0">
                        ...
                        <?php else: ?>
                        <div class="col-sm-6">
                            <form method="post" action="?module=diemdanh&action=index&id=<?= $id_su_kien ?>&lich=<?= $id_lich ?>" id="formDongDiemDanh">
...
function layViTriHienTai() {
    ...
    }, function() { showToast('Không thể lấy vị trí. Hãy nhập thủ công.', 'warning'); });
}
</script>
