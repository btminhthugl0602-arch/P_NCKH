...
if (!$is_btc) { die('Không có quyền truy cập.'); }
...
// Token 6 số cố định suốt phiên (seed: idLich + thời điểm mở)
$now     = time();
$secret  = 'NCKH_DD_' . $lich['idSK'];
$moTime  = $lich['thoiGianMoDiemDanh'] ? strtotime($lich['thoiGianMoDiemDanh']) : $now;
$raw     = hash_hmac('sha256', $id_lich . '_' . $moTime, $secret);
$token   = str_pad((string)(hexdec(substr($raw, 0, 8)) % 1000000), 6, '0', STR_PAD_LEFT);

$checkinUrl = _HOST_URL . "/?module=diemdanh&action=checkin&lich=$id_lich";

// Số đã điểm danh
$daDiemDanh = (int)(mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT COUNT(*) AS c FROM diemdanh WHERE idLichTrinh=$id_lich AND hienDien=1"))['c'] ?? 0);

// Tổng số người thuộc sự kiện
$tongNguoi = (int)(mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT COUNT(DISTINCT tv.idtk) AS c
     FROM thanhviennhom tv
     JOIN nhom n ON tv.idnhom = n.idnhom
     WHERE n.idSK = {$lich['idSK']} AND n.isActive=1 AND tv.trangthai=1"))['c'] ?? 0);
...
<head>
    ...
    <title>Điểm danh — <?= htmlspecialchars($lich['tenHoatDong']) ?></title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .event-sub { color: var(--muted); font-size: .95rem; margin-bottom: 28px; }
        /* ===== QR card ===== */
        .qr-card {
            background: #fff;
            border-radius: 20px;
        ...
        .token-label { font-size: .78rem; color: var(--muted); text-transform: uppercase; letter-spacing: .08em; margin-bottom: 10px; }
        .token-digits {
        ...
        .token-sub { font-size: .78rem; color: var(--muted); margin-top: 8px; }
        /* ===== Stats ===== */
        ...
        .countdown-label { font-size: .82rem; color: var(--muted); }
        .countdown-value { font-size: 1.25rem; font-weight: 700; color: var(--green); min-width: 90px; }
        /* ===== URL ===== */
        .url-bar {
...
<a href="<?= _HOST_URL ?>/?module=diemdanh&action=index&id=<?= $lich['idSK'] ?>&lich=<?= $id_lich ?>"
   class="btn-back">
...
<div class="wrap">
    ...
    <h1 class="event-title"><?= htmlspecialchars($lich['tenHoatDong']) ?></h1>
    <p class="event-sub">
        <?= htmlspecialchars($lich['tenSK']) ?>
        <?php if ($lich['diaDiem']): ?>
        &nbsp;·&nbsp;<i class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($lich['diaDiem']) ?>
        <?php endif; ?>
    </p>
...
const ID_LICH     = <?= $id_lich ?>;
const TONG_NGUOI  = <?= $tongNguoi ?>;
<?php if ($dongTime): ?>const DONG_TIME = <?= $dongTime ?> * 1000;<?php endif; ?>
...
function tickDong() {
    ...
    if (!el) return;
    if (d <= 0) { el.textContent = 'Đã đóng!'; el.style.color = '#ef4444'; setTimeout(()=>location.reload(),1500); return; }
    const m = Math.floor(d/60000), s = Math.floor((d%60000)/1000);
    el.textContent = m + ':' + (s<10?'0':'') + s;
    const ratio = d / (DONG_TIME - Date.now() + d);
    el.style.color = d < 60000 ? '#ef4444' : d < 300000 ? '#f59e0b' : '#22c55e';
}
tickDong(); setInterval(tickDong, 1000);
<?php endif; ?>

// Realtime count mỗi 8 giây
setInterval(function() {
    fetch('<?= _HOST_URL ?>/?module=diemdanh&action=count_ajax&lich=' + ID_LICH)
        .then(r => r.json())
        .then(d => {
