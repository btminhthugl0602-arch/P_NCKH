                    ...
                    <div class="col-12">
                        <div class="tb-detail-card shadow-sm">
                            <div class="tb-detail-header">
                                <div><h5 class="fw-bold text-primary m-0"><?=htmlspecialchars($tb['tenTieuBan'])?></h5><p class="text-muted small m-0 mt-1"><?=$tb['ngayBaoCao'] ? date('d/m/Y', strtotime($tb['ngayBaoCao'])) : '...'?> | Phòng: <?=htmlspecialchars($tb['diaDiem'] ?: '...')?></p></div>
                            ...
                            <div class="tb-detail-body">
                                ...
                                <?php if(empty($sps)): ?><p class="text-muted small fst-italic text-center bg-light p-3 rounded border">Trống.</p><?php else: ?>
                                    <div class="pe-2"><?php foreach($sps as $sp): ?><div class="sp-list-item"><span class="text-dark"><span class="badge bg-secondary me-2"><?=htmlspecialchars($sp['tennhom']?:$sp['manhom'])?></span><strong><?=htmlspecialchars($sp['tensanpham'])?></strong></span><form method="post" class="m-0"><input type="hidden" name="action" value="remove_sp_tb"><input type="hidden" name="idTieuBan" value="<?=$tb_id?>"><input type="hidden" name="idSanPham" value="<?=$sp['idSanPham']?>"><button type="submit" class="btn-remove-sp" title="Rút bài"><i class="bi bi-x-circle-fill fs-5"></i></button></form></div><?php endforeach; ?></div>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
...
?>
<div class="modal fade" id="mEditTB_<?=$tb_id?>"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow"><form method="post"><input type="hidden" name="action" value="edit_tb"><input type="hidden" name="idTieuBan" value="<?=$tb_id?>"><div class="modal-header bg-primary text-white"><h5 class="modal-title">Sửa: <?=htmlspecialchars($tb['tenTieuBan'])?></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body p-4"><div class="mb-3"><label class="fw-bold">Tên tiểu ban</label><input type="text" name="tenTieuBan" class="form-control" value="<?=htmlspecialchars($tb['tenTieuBan'])?>" required></div><div class="row"><div class="col-6 mb-3"><label class="fw-bold">Ngày BC</label><input type="date" name="ngayBaoCao" class="form-control" value="<?=$tb['ngayBaoCao']?>"></div><div class="col-6 mb-3"><label class="fw-bold">Địa điểm</label><input type="text" name="diaDiem" class="form-control" value="<?=htmlspecialchars($tb['diaDiem'])?>"></div></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button><button type="submit" class="btn btn-primary">Lưu</button></div></form></div></div></div>

<div class="modal fade" id="mSp_<?=$tb_id?>"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0"><form method="post"><input type="hidden" name="action" value="add_sp_tb"><input type="hidden" name="idTieuBan" value="<?=$tb_id?>"><div class="modal-header bg-primary text-white"><h5 class="modal-title">Thêm bài vào <?=htmlspecialchars($tb['tenTieuBan'])?></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body p-4"><label class="fw-bold mb-2">Chọn bài (Đã duyệt)</label><select name="idSanPham" class="form-select" required><option value="">-- Chọn --</option><?php foreach($unassigned_sps as $sp): ?><option value="<?=$sp['idSanPham']?>"><?=htmlspecialchars($sp['tennhom'] . ' - ' . $sp['tensanpham'])?></option><?php endforeach; ?></select></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button><button type="submit" class="btn btn-primary">Thêm</button></div></form></div></div></div>

<div class="modal fade" id="mGv_<?=$tb_id?>"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow"><div class="modal-header bg-primary text-white"><h5 class="modal-title">Hội đồng: <?=htmlspecialchars($tb['tenTieuBan'])?></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body p-4 bg-light"><form method="post" class="d-flex gap-2 mb-4 p-3 bg-white border rounded shadow-sm"><input type="hidden" name="action" value="add_gv_tb"><input type="hidden" name="active_tab" value="2"><input type="hidden" name="idTieuBan" value="<?=$tb_id?>"><select name="idGV" class="form-select form-select-sm" required><option value="">-- Thêm Giám khảo --</option><?php foreach($available_gvs as $gv): ?><option value="<?=$gv['idGV']?>"><?=htmlspecialchars($gv['tenGV'])?></option><?php endforeach; ?></select><button type="submit" class="btn btn-primary btn-sm text-nowrap"><i class="bi bi-plus-lg"></i> Thêm</button></form><h6 class="fw-bold mb-2 text-dark">Danh sách hiện tại:</h6><div class="d-flex flex-wrap gap-2"><?php if(empty($gvs)): ?><span class="text-muted small">Chưa có ai.</span><?php else: foreach($gvs as $gv): ?><span class="badge bg-white text-dark border p-2 shadow-sm d-flex align-items-center"><?=htmlspecialchars($gv['tenGV'])?><form method="post" class="m-0 ms-2"><input type="hidden" name="action" value="remove_gv_tb"><input type="hidden" name="active_tab" value="2"><input type="hidden" name="idTieuBan" value="<?=$tb_id?>"><input type="hidden" name="idGV" value="<?=$gv['idGV']?>"><button type="submit" class="btn-close" style="font-size: 0.6rem;"></button></form></span><?php endforeach; endif; ?></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary rounded-pill px-4" data-bs-dismiss="modal">Đóng</button></div></div></div></div>
<?php endforeach; ?>

<div class="modal fade" id="modalCreateTB"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow"><form method="post"><input type="hidden" name="action" value="create_tb"><div class="modal-header bg-dark text-white"><h5 class="modal-title">Khởi tạo Tiểu ban</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body p-4"><div class="mb-3"><label class="fw-bold">Tên tiểu ban *</label><input type="text" name="tenTieuBan" class="form-control" placeholder="Tiểu ban AI" required></div><div class="mb-3"><label class="fw-bold">Thuộc vòng *</label><select name="idVongThi" class="form-select" required><option value="">-- Chọn --</option><?php foreach($vong_array as $v):?><option value="<?=$v['idVongThi']?>"><?=$v['tenVongThi']?></option><?php endforeach;?></select></div><div class="row"><div class="col-6 mb-3"><label class="fw-bold">Ngày BC</label><input type="date" name="ngayBaoCao" class="form-control"></div><div class="col-6 mb-3"><label class="fw-bold">Địa điểm</label><input type="text" name="diaDiem" class="form-control" placeholder="Phòng..."></div></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button><button type="submit" class="btn btn-primary">Khởi tạo</button></div></form></div></div></div>
