...
$tieuban_list = mysqli_fetch_all(mysqli_query($conn, "SELECT tb.*, v.tenVongThi FROM tieuban tb LEFT JOIN vongthi v ON tb.idVongThi = v.idVongThi WHERE tb.idSK = $id_su_kien ORDER BY tb.idTieuBan ASC"), MYSQLI_ASSOC);

$gv_tb_map = [];
$res_gv = mysqli_query($conn, "SELECT tbg.*, gv.tenGV FROM tieuban_giangvien tbg JOIN giangvien gv ON tbg.idGV = gv.idGV");
if ($res_gv) {
    while ($r = mysqli_fetch_assoc($res_gv)) $gv_tb_map[$r['idTieuBan']][] = $r;
}
                ...
                <div class="row mb-4">
                    <div class="col-md-4">
                        <div class="tb-overview-card">
                            <div class="tb-overview-icon bg-primary shadow-sm"><i class="bi bi-collection"></i></div>
                            <div>
                                <h3 class="fw-bold text-dark m-0"><?php echo count($tieuban_list); ?></h3><span class="text-muted small">Tiểu ban đã tạo</span>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="tb-overview-card">
                            <div class="tb-overview-icon bg-success shadow-sm"><i class="bi bi-file-earmark-text"></i></div>
                            <div>
                                <h3 class="fw-bold text-dark m-0"><?php echo count($assigned_sp_ids); ?></h3><span class="text-muted small">Bài thi đã xếp phòng</span>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="tb-overview-card">
                            <div class="tb-overview-icon bg-warning shadow-sm"><i class="bi bi-clock-history text-dark"></i></div>
                            <div>
                                <h3 class="fw-bold text-dark m-0"><?php echo count($unassigned_sps); ?></h3><span class="text-muted small">Bài thi chờ xếp</span>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="tb-detail-card shadow-sm">
                                <div class="tb-detail-header">
                                    <div>
                                        ...
                                        <p class="text-muted small m-0 mt-1"><?= $tb['ngayBaoCao'] ? date('d/m/Y', strtotime($tb['ngayBaoCao'])) : '...' ?> | Phòng: <?= htmlspecialchars($tb['diaDiem'] ?: '...') ?></p>
                                    </div>
                                    <div class="d-flex gap-2"><button class="btn-circle-icon edit" data-bs-toggle="modal" data-bs-target="#mEditTB_<?= $tb_id ?>"><i class="bi bi-pencil-fill"></i></button>
                                        <form method="post" class="m-0" id="formXoaTB-<?= $tb_id ?>"><input type="hidden" name="action" value="delete_tb"><input type="hidden" name="idTieuBan" value="<?= $tb_id ?>"><button type="button" class="btn-circle-icon delete" onclick="showConfirm({title:'Xác nhận xóa tiểu ban',message:'Thao tác này không thể hoàn tác.',type:'danger',confirmText:'Xóa',onConfirm:()=>document.getElementById('formXoaTB-<?= $tb_id ?>').submit()})"><i class="bi bi-trash-fill"></i></button></form>
                                    </div>
                                </div>
                                <div class="tb-detail-body">
                                    <div class="mb-4">
                                        <h6 class="fw-bold text-dark mb-3">Hội đồng Ban giám khảo:</h6>
                                        <div class="d-flex flex-wrap gap-2"><?php if (empty($gvs)): ?><span class="text-muted small fst-italic">Chưa có thành viên nào.</span><?php else: foreach ($gvs as $gv): ?><span class="pill-badge border text-secondary"><?= htmlspecialchars($gv['tenGV']) ?></span><?php endforeach;
                                                                                                                                                                                                                                                                                                    endif; ?></div>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        <h6 class="fw-bold text-dark m-0">Bài báo cáo (<?= count($sps) ?>):</h6><button class="btn btn-outline-primary btn-sm rounded-pill px-3 fw-bold" data-bs-toggle="modal" data-bs-target="#mSp_<?= $tb_id ?>"><i class="bi bi-plus-lg me-1"></i> Thêm bài</button>
                                    </div>
                                    <?php if (empty($sps)): ?><p class="text-muted small fst-italic text-center bg-light p-3 rounded border">Trống.</p><?php else: ?>
                                        <div class="pe-2"><?php foreach ($sps as $sp): ?><div class="sp-list-item"><span class="text-dark"><span class="badge bg-secondary me-2"><?= htmlspecialchars($sp['tennhom'] ?: $sp['manhom']) ?></span><strong><?= htmlspecialchars($sp['tensanpham']) ?></strong></span>
                                                    <form method="post" class="m-0"><input type="hidden" name="action" value="remove_sp_tb"><input type="hidden" name="idTieuBan" value="<?= $tb_id ?>"><input type="hidden" name="idSanPham" value="<?= $sp['idSanPham'] ?>"><button type="submit" class="btn-remove-sp" title="Rút bài"><i class="bi bi-x-circle-fill fs-5"></i></button></form>
...
?>
    <div class="modal fade" id="mEditTB_<?= $tb_id ?>">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                <form method="post"><input type="hidden" name="action" value="edit_tb"><input type="hidden" name="idTieuBan" value="<?= $tb_id ?>">
                    <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title">Sửa: <?= htmlspecialchars($tb['tenTieuBan']) ?></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body p-4">
                        <div class="mb-3"><label class="fw-bold">Tên tiểu ban</label><input type="text" name="tenTieuBan" class="form-control" value="<?= htmlspecialchars($tb['tenTieuBan']) ?>" required></div>
                        <div class="row">
                            <div class="col-6 mb-3"><label class="fw-bold">Ngày BC</label><input type="date" name="ngayBaoCao" class="form-control" value="<?= $tb['ngayBaoCao'] ?>"></div>
                            <div class="col-6 mb-3"><label class="fw-bold">Địa điểm</label><input type="text" name="diaDiem" class="form-control" value="<?= htmlspecialchars($tb['diaDiem']) ?>"></div>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="mSp_<?= $tb_id ?>">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0">
                <form method="post"><input type="hidden" name="action" value="add_sp_tb"><input type="hidden" name="idTieuBan" value="<?= $tb_id ?>">
                    <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title">Thêm bài vào <?= htmlspecialchars($tb['tenTieuBan']) ?></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body p-4"><label class="fw-bold mb-2">Chọn bài (Đã duyệt)</label><select name="idSanPham" class="form-select" required>
                            <option value="">-- Chọn --</option><?php foreach ($unassigned_sps as $sp): ?><option value="<?= $sp['idSanPham'] ?>"><?= htmlspecialchars($sp['tennhom'] . ' - ' . $sp['tensanpham']) ?></option><?php endforeach; ?>
                        </select></div>
    ...
    <div class="modal fade" id="mGv_<?= $tb_id ?>">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">Hội đồng: <?= htmlspecialchars($tb['tenTieuBan']) ?></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4 bg-light">
                    <form method="post" class="d-flex gap-2 mb-4 p-3 bg-white border rounded shadow-sm"><input type="hidden" name="action" value="add_gv_tb"><input type="hidden" name="active_tab" value="2"><input type="hidden" name="idTieuBan" value="<?= $tb_id ?>"><select name="idGV" class="form-select form-select-sm" required>
                            <option value="">-- Thêm Giám khảo --</option><?php foreach ($available_gvs as $gv): ?><option value="<?= $gv['idGV'] ?>"><?= htmlspecialchars($gv['tenGV']) ?></option><?php endforeach; ?>
                        </select><button type="submit" class="btn btn-primary btn-sm text-nowrap"><i class="bi bi-plus-lg"></i> Thêm</button></form>
                    <h6 class="fw-bold mb-2 text-dark">Danh sách hiện tại:</h6>
                    <div class="d-flex flex-wrap gap-2"><?php if (empty($gvs)): ?><span class="text-muted small">Chưa có ai.</span><?php else: foreach ($gvs as $gv): ?><span class="badge bg-white text-dark border p-2 shadow-sm d-flex align-items-center"><?= htmlspecialchars($gv['tenGV']) ?><form method="post" class="m-0 ms-2"><input type="hidden" name="action" value="remove_gv_tb"><input type="hidden" name="active_tab" value="2"><input type="hidden" name="idTieuBan" value="<?= $tb_id ?>"><input type="hidden" name="idGV" value="<?= $gv['idGV'] ?>"><button type="submit" class="btn-close" style="font-size: 0.6rem;"></button></form></span><?php endforeach;
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    endif; ?></div>
                </div>
                <div class="modal-footer"><button type="button" class="btn btn-secondary rounded-pill px-4" data-bs-dismiss="modal">Đóng</button></div>
            </div>
        </div>
    </div>
<?php endforeach; ?>
...
<div class="modal fade" id="modalCreateTB">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <form method="post"><input type="hidden" name="action" value="create_tb">
                <div class="modal-header bg-dark text-white">
                    <h5 class="modal-title">Khởi tạo Tiểu ban</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <div class="mb-3"><label class="fw-bold">Tên tiểu ban *</label><input type="text" name="tenTieuBan" class="form-control" placeholder="Tiểu ban AI" required></div>
                    <div class="mb-3"><label class="fw-bold">Thuộc vòng *</label><select name="idVongThi" class="form-select" required>
                            <option value="">-- Chọn --</option><?php foreach ($vong_array as $v): ?><option value="<?= $v['idVongThi'] ?>"><?= $v['tenVongThi'] ?></option><?php endforeach; ?>
                        </select></div>
                    <div class="row">
                        <div class="col-6 mb-3"><label class="fw-bold">Ngày BC</label><input type="date" name="ngayBaoCao" class="form-control"></div>
                        <div class="col-6 mb-3"><label class="fw-bold">Địa điểm</label><input type="text" name="diaDiem" class="form-control" placeholder="Phòng..."></div>
                    </div>
                </div>
                <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button><button type="submit" class="btn btn-primary">Khởi tạo</button></div>
            </form>
        </div>
    </div>
</div>
