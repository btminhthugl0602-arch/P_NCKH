                                                                    ...
                                                                    <?php if ($sp['isWarning']): ?>
                                                                        ...
                                                                        <div class="bg-white p-3 rounded border border-warning shadow-sm">
                                                                            ...
                                                                            <form method="post" class="d-flex align-items-center gap-2 flex-wrap">
                                                                                ...
                                                                                <select name="idGV" id="idGV-<?=$sp['idSanPham']?>" class="form-select border-warning" style="max-width: 300px;" required>
                                                                                ...
                                                                                <span id="errGVSelect-<?=$sp['idSanPham']?>" class="text-danger small d-none">Vui lòng chọn Trọng tài!</span>
                                                                                <button type="button" class="btn btn-warning text-dark fw-bold px-4 shadow-sm btn-moi-gk" data-idsp="<?=$sp['idSanPham']?>">
...
<script>
document.querySelectorAll('.btn-moi-gk').forEach(function (btn) {
    btn.addEventListener('click', function () {
        const idSP   = this.dataset.idsp;
        const form   = this.closest('form');
        const select = document.getElementById('idGV-' + idSP);
        const errEl  = document.getElementById('errGVSelect-' + idSP);
        ...
        showConfirm({
            title      : 'Xác nhận mời giám khảo',
            message    : 'Hệ thống sẽ cập nhật lại tiến độ chấm của bài thi. Bạn có chắc chắn muốn mời thêm giám khảo này?',
            type       : 'warning',
