...
$can_submit_product = ($userId > 0) ? kiem_tra_quyen_su_kien($conn, $userId, $id, 'nop_san_pham') : false;
...
$is_creator = ($userId > 0 && isset($event['idNguoiTao']) && $userId == $event['idNguoiTao']);
...
if ($id_gv_logged > 0) {
    $sql_check_pc = "
        SELECT 1 FROM phancong_doclap pcd JOIN vongthi v ON pcd.idVongThi = v.idVongThi WHERE pcd.idGV = $id_gv_logged AND v.idSK = $id
        UNION
        SELECT 1 FROM tieuban_giangvien tbg JOIN tieuban tb ON tbg.idTieuBan = tb.idTieuBan WHERE tbg.idGV = $id_gv_logged AND tb.idSK = $id
    ";
...
if (isset($_POST['create_group']) && $userId > 0) {
    ...
    if (!$can_create_group) {
        $_SESSION['flash_msg']  = 'Chỉ sinh viên mới được tạo nhóm.';
    ...
    if (!empty($tennhom)) {
        $result = tao_nhom_moi($conn, $userId, $id, $tennhom, $mota, $soluongtoida);
        if ($result['status']) {
...
if (isset($_POST['xin_vao_nhom']) && $userId > 0) {
    if (!$can_join_group) {
        $_SESSION['flash_msg']  = 'Chỉ sinh viên mới được xin vào nhóm.';
        $_SESSION['flash_type'] = 'warning';
        header("Location: " . $_SERVER['REQUEST_URI']);
        exit();
    }
...
<style>
    ...
    .animate-pulse { animation: pulse-badge 1.4s ease-in-out infinite; }
</style>
                                                                                ...
                                                                                <div class="course-card">
                                                                                    ...
                                                                                    <div class="course-content">
                                                                                        ...
                                                                                        <div class="mt-2 d-flex gap-2 flex-wrap align-items-center">
                                                                                            ...
                                                                                            <?php elseif (!$isFull && $g['dangtuyen'] && $userId > 0 && $can_join_group): ?>                                                                                                <button type="button" class="btn-course"
                                                        ...
                                                        <div class="col-lg-6 col-md-6 my-group-item" data-ten="<?= htmlspecialchars(strtolower($g['tennhom'])) ?>">
                                                            <div class="nhom-card">
                                                                ...
                                                                <div class="nhom-actions">
                                                                    <a href="<?= _HOST_URL ?>?module=event&action=chitiethom&id=<?= $gId ?>" class="btn-nhom btn-nhom-view">
                                ...
                                <div class="tab-pane fade" id="event-attendance" role="tabpanel">
                                    <div class="mt-4">
                                        <?php if ($phien_dang_mo): ?>
                                            <div class="alert <?= $da_diemdanh ? 'alert-success' : 'alert-warning' ?> border-0 shadow-sm d-flex align-items-center gap-3 mb-4 p-3">
                                                <i class="bi bi-<?= $da_diemdanh ? 'check-circle-fill' : 'unlock-fill' ?> fs-3 flex-shrink-0"></i>
                                                <div class="flex-grow-1">
                                                    <?php if ($da_diemdanh): ?>
                                                        <strong class="d-block">Bạn đã điểm danh buổi này ✓</strong>
                                                        <small class="text-muted">Hoạt động: <?= htmlspecialchars($phien_dang_mo['tenHoatDong']) ?></small>
                                                    <?php else: ?>
                                                        <strong class="d-block">Đang có phiên điểm danh mở!</strong>
                                                        <small>
                                                            <?= htmlspecialchars($phien_dang_mo['tenHoatDong']) ?>
                                                            &nbsp;—&nbsp; Đóng lúc <?= date('H:i', strtotime($phien_dang_mo['thoiGianDongDiemDanh'])) ?>
                                                        </small>
                                                    <?php endif; ?>
                                                </div>
                                                <?php if (!$da_diemdanh): ?>
                                        ...
                                        <?php else: ?>
                                            <div class="alert alert-light border text-muted d-flex align-items-center gap-3 mb-4 p-3">
                                                <i class="bi bi-lock fs-3 flex-shrink-0 text-secondary"></i>
                                                <div>
                                                    <strong class="d-block text-dark">Chưa có phiên điểm danh nào đang mở</strong>
                                                    <small>Ban tổ chức sẽ mở điểm danh trước mỗi buổi tổ chức. Bạn sẽ thấy thông báo tại đây.</small>
                                                </div>
                                            </div>
                                        <?php endif; ?>
                                            ...
                                            <div class="card border-0 bg-light">
                                                <div class="card-body d-flex align-items-center justify-content-between gap-3">
                                                    <div>
                                                        ...
                                                        <small class="text-muted">Tạo lịch trình, mở/đóng phiên, điểm danh thủ công, xem danh sách</small>
                                                    </div>
                                        ...
                                        <div class="d-flex flex-column gap-3 mt-4">
                                            ...
                                            <div class="card border-0 shadow-sm bg-light mt-2">
                                                <div class="card-body">
                                                    ...
                                                    <a class="btn btn-success text-white w-100 text-start p-3 mb-2 rounded-3 shadow-sm" href="<?= _HOST_URL ?>/?module=event&action=config_grading&id=<?= $id ?>">
                                                        <i class="bi bi-pencil-square fs-4 me-2 align-middle"></i>
                                                        <strong>1. Phân công & Quản lý Điểm (Sơ loại)</strong>
                                                    </a>
                                                    <a class="btn btn-warning text-dark w-100 text-start p-3 rounded-3 shadow-sm" href="<?= _HOST_URL ?>/?module=event&action=config_subcommittee&id=<?= $id ?>">
                            ...
                            <?php if ($is_btc || $is_giangvien || $can_grading_area): ?>
                                <div class="tab-pane fade" id="event-grading" role="tabpanel">
                                    <div class="event-grading-content mt-4">
                                        ...
                                        <div class="d-flex flex-column gap-3 mt-4">
                                            <a class="btn btn-success text-start p-3 shadow-sm rounded-3" href="<?= _HOST_URL ?>/?module=event&action=my_grading_tasks&id=<?= $id ?>">
